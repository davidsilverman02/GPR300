...
namespace ew {
	...
	glm::vec3 quickTangentQuad(glm::vec3 basePos0, glm::vec3 basePos1, glm::vec3 basePos2, glm::vec3 basePos3, glm::vec2 uv0, glm::vec2 uv1, glm::vec2 uv2, glm::vec2 uv3)
	{
		return (quickTangent(basePos0, basePos2, basePos1, uv0, uv2, uv1) + quickTangent(basePos0, basePos3, basePos2, uv0, uv3, uv2)) / 2.0f;
	}

	glm::vec3 quickTangentScreen(glm::vec3 basePos0, glm::vec3 basePos1, glm::vec3 basePos2, glm::vec3 basePos3, glm::vec2 uv0, glm::vec2 uv1, glm::vec2 uv2, glm::vec2 uv3)
	{
		return (quickTangent(basePos0, basePos1, basePos2, uv0, uv1, uv2) + quickTangent(basePos0, basePos2, basePos3, uv0, uv2, uv3)) / 2.0f;
	}
	...
	void createPlane(float width, float height, MeshData& meshData) {
		...
		glm::vec3 find = quickTangentQuad(vertices[0].position, vertices[1].position, vertices[2].position, vertices[3].position, vertices[0].uv, vertices[1].uv, vertices[2].uv, vertices[3].uv);
	...
	void createQuad(float width, float height, MeshData& meshData) {
		...
		meshData.vertices.assign(&vertices[0], &vertices[4]);
		glm::vec3 find = quickTangentScreen(vertices[0].position, vertices[1].position, vertices[2].position, vertices[3].position, vertices[0].uv, vertices[1].uv, vertices[2].uv, vertices[3].uv);
	...
	void createSphere(float radius, int numSegments, MeshData& meshData)
	{
		...
		for (int i = 1; i < numSegments; i++)
		{
			...
			for (int j = 0; j <= numSegments; ++j)
			{
				...
				glm::vec2 uv = glm::vec2(j / (float)(numSegments), 1 - (i / (float)(numSegments)));
