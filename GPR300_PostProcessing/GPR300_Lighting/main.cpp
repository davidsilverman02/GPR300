...
int main() {
	...
	Shader postProcessing("shaders/MRT.vert", "shaders/MRT.frag");
	...
	for (int i = 0; i < 1; i++)
	{
		syst.dLights[i].color = glm::vec3(0, 0, 0);
		syst.dLights[i].direction = glm::normalize(glm::vec3(0, 0, 0));
		syst.dLights[i].intensity = 0;
	...
	//postProcessing.setInt("screen", 0);
