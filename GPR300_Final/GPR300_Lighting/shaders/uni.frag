...
float calcShadow(sampler2D shadowMap, vec4 lightSpacePosition, float bias)
{
    float totalShadow = 0;
    ...
    float smDepth;
    float zDepth = coord.z - bias;
    //= texture(shadowMap, coord.xy).r;
    for(int x = -1; x <= 1; ++x)
{
    for(int y = -1; y <= 1; ++y)
    {
        smDepth = texture(shadowMap, coord.xy + vec2(x, y) * texelOffset).r; 
        totalShadow += step(smDepth, zDepth);     
    }    
}
    totalShadow /= 9.0;
...
void main(){
    ...
    float bias = max(maxBias * (1.0 - dot(mesoNormal,_ToLight)), minBias);
    vec4 tex1 = texture(_SandTexture, vs_out.UV);
    vec3 vecA = vec3(tex1).rgb;
    // * (1.0 - calcShadow(_ShadowTexture, lightSpacePos, bias))
    vec3 blinnPhong = getAmbient() + (getDiffuse(mesoNormal) + getSpecular(mesoNormal)) * (1.0 - calcShadow(_ShadowTexture, lightSpacePos, bias));
